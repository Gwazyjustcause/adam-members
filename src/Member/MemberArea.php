...
final class MemberArea {
	...
	private function render_apd_registration_form( Member $member, array $fields, string $message ): string {
		...
		ob_start(); ?>
		<section class="adam-public-form adam-card" data-adam-membership-form="apd-association"><div class="adam-card-heading"><p class="adam-eyebrow">ADAM / ANA</p><h2><?php esc_html_e( 'Associar APD através da ADAM', 'adam-membership' ); ?></h2></div><?php echo wp_kses_post( $message ); ?><p><strong><?php esc_html_e( 'Valor aplicável:', 'adam-membership' ); ?> <?php echo esc_html( number_format_i18n( (float) $price, 2 ) . ' €' ); ?></strong></p>
		...
		<div class="adam-form-section"><h3><?php esc_html_e( 'Fotografia', 'adam-membership' ); ?></h3><?php if ( $photo_id && wp_get_attachment_url( $photo_id ) ) : ?><p><img src="<?php echo esc_url( (string) wp_get_attachment_url( $photo_id ) ); ?>" alt="" style="max-width:180px;height:auto"></p><?php endif; ?><label class="adam-form-field"><span><?php esc_html_e( 'Substituir fotografia', 'adam-membership' ); ?></span><input type="file" name="profile_photo" accept=".jpg,.jpeg,.png,.webp"></label><label><input type="checkbox" name="remove_profile_photo" value="1"> <?php esc_html_e( 'Remover fotografia', 'adam-membership' ); ?></label></div>
		<div class="adam-form-section"><label class="adam-form-field"><span><?php esc_html_e( 'Comprovativo de pagamento', 'adam-membership' ); ?></span><input type="file" name="payment_receipt" accept=".pdf,.jpg,.jpeg,.png" required></label></div><button class="button button-primary" name="adam_apd_association_submit" value="1"><?php esc_html_e( 'Submeter pedido', 'adam-membership' ); ?></button></form></section>
		<?php $html = (string) ob_get_clean(); $html .= '<script>(function(){var f=document.querySelector("[data-adam-membership-form=apd-association]");if(!f)return;var r=f.querySelectorAll("input[name=apd_information_mode]"),e=f.querySelectorAll("input[name],select[name]");function s(){var x=f.querySelector("input[name=apd_information_mode]:checked"),l=!x||x.value!=="edit";e.forEach(function(i){if(i.name!=="apd_information_mode"&&i.name!=="payment_receipt"){i.readOnly=l;i.disabled=l;i.classList.toggle("adam-apd-field--locked",l);}});}r.forEach(function(i){i.addEventListener("change",s);});s();}());</script>'; return $html;
	}
