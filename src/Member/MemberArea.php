	...
	private function render_apd_association_page( Member $member ): string {
		...
		if ( 'POST' === strtoupper( $_SERVER['REQUEST_METHOD'] ?? '' ) && isset( $_POST['adam_apd_association_submit'] ) ) {
			...
			elseif ( ! in_array( (string) ( $_POST['apd_information_mode'] ?? '' ), array( 'confirm', 'edit' ), true ) ) { $message = $this->notice_markup( 'error', __( 'Selecione se os seus dados estão corretos ou se precisam de alterações.', 'adam-membership' ) ); }
				...
				$payload = array(); foreach ( $fields as $key => $field ) { $payload[ $key ] = $field['editable'] && 'edit' === (string) $_POST['apd_information_mode'] ? sanitize_text_field( wp_unslash( $_POST[ $key ] ?? $field['value'] ) ) : $field['value']; }
				$payload['association_name'] = sanitize_text_field( wp_unslash( $_POST['association_name'] ?? $member->field( 'adam_external_association_name' ) ) ); $payload['external_member_number'] = sanitize_text_field( wp_unslash( $_POST['external_member_number'] ?? $member->field( 'adam_external_member_number' ) ) );
		...
		return $this->render_apd_clean_form( $member, $fields, $message );
