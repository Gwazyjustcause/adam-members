...
<div class="wrap adam-membership-admin adam-admin-page">
	...
	<?php if ( 'migrated' === $notice ) : ?><div class="notice notice-success is-dismissible"><p><?php esc_html_e( 'A pÃ¡gina Associa-te foi migrada para blocos Gutenberg nativos.', 'adam-membership' ); ?></p></div><?php endif; ?>
	...
	?>
			...
			if ( $row['exists'] ) :
				?>
				<?php if ( ! empty( $row['legacy'] ) ) : ?><a class="button button-primary" href="<?php echo esc_url( wp_nonce_url( add_query_arg( 'action', 'adam_membership_migrate_landing_content', admin_url( 'admin-post.php' ) ), 'adam_membership_migrate_landing_content' ) ); ?>"><?php esc_html_e( 'Migrar para Gutenberg', 'adam-membership' ); ?></a><?php endif; ?>
