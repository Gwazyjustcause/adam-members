...
$row = static fn ( string $member, string $year, string $movement, string $id, string $date = '2026-08-14', string $amount = '12.00', string $method = 'Transferência bancária' ): array => array( $member, 'Member ' . $member, $year, $movement, 'Efetivo', $amount, $date, $method, 'Pago', $id, '' );
...
adam_movement_assert( 'registration:reg-1' === $edited_registration[9], 'Edited data preserves the canonical ID.' );
...
adam_movement_assert( str_contains( $client, "read_values( 'A5:K', \$request_id )" ), 'Update re-reads current rows by canonical ID.' );
