...
foreach ( array( 'sync_service_entered', 'read_values_started', 'read_values_completed', 'append_started', 'append_completed', 'metadata_read_started', 'metadata_read_completed', 'confirmation_started', 'confirmation_completed', 'financial_retry_post_received', 'financial_retry_handler_entered', 'financial_retry_validation_passed', 'financial_retry_sync_service_called', 'financial_retry_sync_service_returned', 'Google Sheets diagnostics build loaded.', 'Google Sheets sheet-id-fix-v2 loaded.' ) as $marker ) {
	adam_diagnostics_assert( ! str_contains( $sync, $marker ) && ! str_contains( $client, $marker ) && ! str_contains( $plugin, $marker ) && ! str_contains( $admin, $marker ), "Temporary trace {$marker} was removed." );
}
