...
adam_private_documents_assert( str_contains( $email, '$this->render_template( $title, $content )' ) && str_contains( $email, "'private_document'" ), 'Private-document email must use the standard branded renderer and normal HTML send pipeline.' );
...
adam_private_documents_assert( str_contains( $plugin, "private const BUILD_ID = 'private-document-trace-v2'" ) && str_contains( $plugin, 'admin-post hooks registered.' ) && str_contains( $admin, 'Plugin::build_id()' ), 'The loaded build must expose an unambiguous bootstrap diagnostic marker.' );
